...
class WebDataset(Dataset):
    ...
    def init_configure(self, transform, mode, pred, probability):
        ...
        if self.mode == 'test':
            self.test_imgs = []
            with open(self.annotation_path + 'test_label0.txt','r') as f:
                lines = f.read().splitlines()
                for l in lines:
                    entry = l.split()           
                    img_path = self.annotation_path + 'test/%s' % (entry[0])
                    self.test_imgs.append(img_path)
        elif self.mode == 'web_test':
            self.web_test_imgs = []
            with open(self.annotation_path + 'test_label1.txt','r') as f:
                lines = f.read().splitlines()
                for l in lines:
                    entry = l.split()           
                    img_path = self.annotation_path + 'test1/%s' % (entry[0])
                    self.web_test_imgs.append(img_path)
        else:
            train_imgs = []
            noise_label = []
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            img_path = self.web_test_imgs[index]
            image = Image.open( img_path ).convert('RGB')
            img = self.transform(image)
            return img
